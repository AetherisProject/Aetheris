...
fn gitignore_never_ignores_gitleaks_toml() {
    ...
    assert!(
        !content.contains(".gitleaks.toml"),
        "gitleaks should NOT be ignored"
    );
}
...
fn no_pem_or_key_files_committed() {
    ...
    assert!(
        !Path::new(".env").exists() || !std::fs::metadata(".env").unwrap().permissions().readonly()
    );
}
...
fn token_scope_documented() {
    ...
    assert!(
        memory.contains("Token scope note"),
        ".memory must note workflow scope"
    );
}
